...
const Galaxy3D: React.FC = () => {
  ...
  useEffect(() => {
    ...
    function animate() {
      ...
      for (const p of planets) {
        ...
        const distFromCenter = Math.sqrt((p.x - sun.x) ** 2 + (p.y - sun.y) ** 2 + (p.z - sun.z) ** 2);
      ...
      for (const p of rotatedPlanets) {
        ...
        if (planetSize > 0 && screenX > -100 && screenX < WIDTH + 100 && screenY > -100 && screenY < HEIGHT + 100) {
          // Draw planet
          ctx.beginPath();
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
      <h1 style={{ color: '#ffb300', fontWeight: 700, fontSize: 32, margin: '24px 0 8px 0', letterSpacing: 1 }}>3D Galaxy Simulator</h1>
      <div style={{ color: '#fff', fontSize: 16, marginBottom: 8, opacity: 0.8 }}>Experience planets orbiting in true 3D space! Watch as they move in and out of the screen with realistic depth.</div>
      ...
      <div style={{ width: '100%', maxWidth: WIDTH, aspectRatio: '1 / 1', background: 'transparent', borderRadius: 16, overflow: 'hidden', boxShadow: '0 2px 16px rgba(0,0,0,0.12)' }}>
        <canvas ref={canvasRef} width={WIDTH} height={HEIGHT} style={{ width: '100%', height: '100%', background: '#0a0a15', borderRadius: 16, display: 'block' }} />
      </div>
    </div>
  );
};
